/**
 * MeiMei Checklist local SQLite — schema aligned with checklist repo local_store.initialize_local_store.
 */
 ...
 */
export function openChecklistDatabase(dbPath) {
 ...
 */
export function getChecklistDb(dbPath) {
  let db = pool.get(dbPath);
  if (!db) {
    db = openChecklistDatabase(dbPath);
    pool.set(dbPath, db);
  }
  return db;
}

export function clearChecklistDbPool() {
