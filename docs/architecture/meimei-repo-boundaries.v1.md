...
- **Surface & config:** `dashboard-surface.mjs`, `miniapp-registry.mjs`, `page-layout.mjs`, `runtime.mjs`, **`kernel-app-registry.mjs`** (external app install registry — MM-KERNEL-202), **`meimei-app-manifest-validate.mjs`** (manifest schema subset — MM-KERNEL-201), **`kernel-external-app-dispatch.mjs`** (opt-in POST dispatch for registered external apps — MM-KERNEL-501)
...
- **HTTP / policy primitives:** `api-channel-adapter.mjs`, `external-channel-policy-engine.mjs`, `imessage-adapter.mjs`, `reliability-telemetry.mjs`, `audit-trail.mjs`, **`openclaw-routing-preview.mjs`** (deterministic routing preview — parity with `scripts/oc-agent --route-only`)
- **Legacy inference (until retired):** `llm.mjs`
- **Checklist (integration surface):** `checklist-api-shell.mjs`, `checklist-local-integration.mjs`, **`checklist-bridge-http.mjs`** (HTTP adapter), `checklist-bridge.mjs`, **`checklist-meimei-trace.mjs`** (monitor ledger for Node LLM), `checklist-node/*` — **integration-adjacent**
- **Platform GET catalog HTML:** **`platform-pages/chrome.mjs`** (shared nav, flashcard, list helper — **K2**); `platform-pages/catalog-pages.mjs` (Apps / Tools / knowmore shells); **`platform-pages/system-monitor-page.mjs`** (queue explorer); **`platform-pages/tool-surface-pages.mjs`** (AI routing preview, API channel adapter shell, AI SDR analytics, Supabase connector, environment variables); **`platform-pages/reference-app-pages.mjs`** (Reference app 1 & 2 demo UIs); **`platform-pages/ops-tool-pages.mjs`** (Inbox, Memory, Mission Control — main + settings shells); **`platform-pages/gtm-pages.mjs`** (Lead enrichment, Lead outreach — main + settings shells); **`platform-pages/reader-pages.mjs`** (What next main + settings, Explain it URL summary + settings, Daily briefing); **`platform-pages/routing-settings-pages.mjs`** (AI routing + API access **settings** shells — main tool pages remain in **`tool-surface-pages.mjs`**); **`platform-pages/home-admin-pages.mjs`** (home dashboard shell, admin/settings page, admin layout editor section — injects chrome from server deps). Future siblings stay under `platform-pages/`; **must not** import from `apps/*`.
