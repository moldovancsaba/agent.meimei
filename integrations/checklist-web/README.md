...
npm run checklist:queue-consumer
```
...
npm run checklist:sync-theme -- --patch-layout
```
...
The **original checklist** flow (competitor snapshots, Mongo insights/recommendations, checklist cards) lives in the Next app at **`/original-checklist`** (footer: “MeiMei weekly pipeline”). LLM calls can use the **same MeiMei Ollama gateway** as the Checklist bridge so routing, secrets, and Ollama stay in one place on the Mac.
...
- **Know More / Neon queue / SQLite worker** — unchanged: use `APP_QUEUE_BASE_URL`, `npm run checklist:queue-consumer`, and the hosted checklist URL as documented above. That path does **not** use the Mongo weekly pipeline.

### HTTP bridge (worker mode)

MeiMei exposes **`/api/checklist/bridge`** (plus your `MEIMEI_PUBLIC_PREFIX`, e.g. `/dashboard`). Point **`AGENT_API_BASE_URL`** at that URL (no trailing path beyond `/bridge`). Set **`MEIMEI_CHECKLIST_SHARED_SECRET`** on the MeiMei host; the hosted Next app must send **`x-meimei-checklist-secret`** with the same value on bridge requests.
