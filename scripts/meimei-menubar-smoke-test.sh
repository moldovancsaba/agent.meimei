#!/usr/bin/env bash
# Build MeiMei Control.app and verify bundle + that the process can start (GUI session).
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
npm run menubar:build
APP="$ROOT/macos/MeiMei/build/MeiMei Control.app"
...
open -gj "$APP"
sleep 2
if pgrep -f 'MeiMei Control.app/Contents/MacOS/MeiMei' >/dev/null 2>&1; then
