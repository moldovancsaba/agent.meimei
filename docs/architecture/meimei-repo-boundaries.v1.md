...
- **Platform GET catalog HTML:** `platform-pages/catalog-pages.mjs` (Apps / Tools / knowmore shells); **`platform-pages/system-monitor-page.mjs`** (queue explorer). Future siblings stay under `platform-pages/`; **must not** import from `apps/*`.
...
- Moving every remaining GET/legacy handler out of `server.mjs` into `platform-pages/*` or apps — **incremental**; Apps / Tools / knowmore catalog HTML is extracted first.
