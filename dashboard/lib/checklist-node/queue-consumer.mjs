/**
 * Neon queue consumer for MeiMei Checklist (Node engine): mirrors checklist
 ...
 */
import { resolveChecklistSqlitePath, isNodeChecklistEngine } from "./engine.mjs";
...
export async function runQueueConsumer(opts) {
  if (!isNodeChecklistEngine()) {
    throw new Error(
      "Node engine required. Unset MEIMEI_CHECKLIST_ENGINE or set to node. For Python, run checklist repo scripts/worker_queue_consumer.py."
    );
  }
  ...
  const dbPath = resolveChecklistSqlitePath(opts.repoRoot);

  console.log(`MeiMei Checklist queue consumer (node) polling ${apiBase}/api/worker/jobs/claim`);
