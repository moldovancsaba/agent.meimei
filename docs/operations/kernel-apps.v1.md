...
2. Run `npm run kernel:validate-app-manifest` on the new tree; update registry **sources** (fragments / order / manifest) and run **`npm run kernel:registry:generate`**, then **`npm run kernel:registry:drift-check`** (or register-only external flow with no legacy row).
