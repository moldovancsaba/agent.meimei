#!/usr/bin/env node
/**
 * Run the Neon-backed queue loop for MeiMei Checklist Node engine (same contract as
...
import { runQueueConsumer } from "../dashboard/lib/checklist-node/queue-consumer.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const repoRoot = path.resolve(__dirname, "..");

const engine = String(process.env.MEIMEI_CHECKLIST_ENGINE || "node").toLowerCase();
if (engine !== "node") {
  console.error(
    "This consumer only runs the in-MeiMei Checklist Node engine. Set MEIMEI_CHECKLIST_ENGINE=node (or unset).\n" +
